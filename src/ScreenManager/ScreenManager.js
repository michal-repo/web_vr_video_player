...
export function registerMeshPanel(ui_name, save_as_name, mode, screen_type, eye) {
	...
	for (const [name, elements] of Object.entries(panels)) {
		if (name === ui) {
			...
			elements.panels.forEach((panel) => {
				...
				positionsToSaveForVRExit[name][panel.save_as_name]["position"] = MAIN[name][panel.ui_name].position.clone();
				positionsToSaveForVRExit[name][panel.save_as_name]["rotation"] = MAIN[name][panel.ui_name].rotation.clone();
			});
		}
	}
}
...
export function resetPosition(ui) {
	for (const [name, elements] of Object.entries(panels)) {
		if (name === ui) {
			elements.panels.forEach((panel) => {
				MAIN[name][panel.ui_name].position.set(positionsToSaveForVRExit[name][panel.save_as_name]["position"].x, positionsToSaveForVRExit[name][panel.save_as_name]["position"].y, positionsToSaveForVRExit[name][panel.save_as_name]["position"].z);
				MAIN[name][panel.ui_name].rotation.set(positionsToSaveForVRExit[name][panel.save_as_name]["rotation"].x, positionsToSaveForVRExit[name][panel.save_as_name]["rotation"].y, positionsToSaveForVRExit[name][panel.save_as_name]["rotation"].z, positionsToSaveForVRExit[name][panel.save_as_name]["rotation"].order);
				positionsToSaveForVRExit[name][panel.save_as_name]["rotation"] = MAIN[name][panel.ui_name].rotation.clone();
			});
		}
	}
